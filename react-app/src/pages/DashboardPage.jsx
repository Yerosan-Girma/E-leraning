...
function statusClass(status) {
  if (status === "approved" || status === "graded") return "success";
...
export default function DashboardPage() {
  ...
  const enrollments = useMemo(() => getEnrolledCourses(), []);
  const payments = useMemo(() => getCoursePayments(), []);
  ...
  const totalLessons = useMemo(() => {
    return enrollments.reduce((accumulator, enrollment) => {
      const course = COURSES_BY_ID[Number(enrollment.id)];
  ...
  const activatePendingEnrollment = (courseId) => {
    const existing = enrollments.find((item) => Number(item.id) === Number(courseId));
    if (!existing) return;
    ...
    notify("Enrollment approved for demo workflow.", "success");
              ...
              {enrollments.length === 0 ? (
                ...
                <div className="row">
                  {enrollments.map((enrollment) => {
                    ...
                    return (
                      <div className="col-md-6 mb-3" key={`course-${enrollment.id}`}>
                          ...
                          <div className="card-body d-flex flex-column">
                            ...
                            <div className="d-flex gap-2 mt-auto">
                              {enrollment.status === "approved" ? (
                                <Link to={`/learning/${enrollment.id}`} className="btn btn-primary btn-sm">
                                  Continue
                                </Link>
                              ) : (
                                <button
                                  ...
                                  onClick={() => activatePendingEnrollment(enrollment.id)}
                                >
                                  Approve Demo
                                </button>
                              )}
              ...
              {payments.length === 0 ? (
                ...
                payments.slice(0, 5).map((payment) => (
                  <div className="border rounded p-2 mb-2" key={payment.id}>
                    <div className="small fw-bold">{payment.courseTitle}</div>
                    <div className="small text-muted">
                      {formatPrice(payment.amount)} via {payment.method}
                    </div>
